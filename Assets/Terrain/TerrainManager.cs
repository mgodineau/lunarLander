...
public class TerrainManager : MonoBehaviour
{
	...
	[SerializeField]
	private float objCosThreshold = 0.05f;
	...
	private void UpdateObjetDisplay(LocalizedObject obj) {
		...
		// création et MAJ de l'objet si il est visible
		if (normalCos < objCosThreshold)
		{
			bool instanceExists = objToPrefInstance.ContainsKey(obj);
			...
			if( !instanceExists ) {
				...
				xId = (int)(sampleCount * angle / 360.0f);

				localRatio = obj.flattenTerrain ? 0.5f : (angle * sampleCount / 360.0f) % 1.0f;
				...
				localRatio = obj.flattenTerrain ? 0.5f : (x % (_terrainWidth / sampleCount)); //TODO tester ce machin
			}
			...
			// aplatissement du terrain si nécessaire
		...
		//Destruction de l'objet si il n'est plus visible
