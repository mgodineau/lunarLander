...
public class Map : InstrumentBehaviour, ISliceLineView, IObjectsView
{
	...
	new private void Start() {
		...
		TerrainManager.Instance.AddSliceLineView(this);
	}
	...
	protected override void BuildUI() {
		...
		UpdateSliceLine(sliceNormal);
		foreach( LocalizedObject obj in knownObjectToMarker.Keys ) {
			UpdateObject(obj);
		}
	}
	...
	public void UpdateObject( LocalizedObject obj ) {
		if( !knownObjectToMarker.ContainsKey(obj) ) {
			return;
		}
		knownObjectToMarker[obj].points = CreateSquareLine(
			LocalToGlobal(dirToLocalPos(obj.Position)) , 
			TerrainManager.Instance.IsObjectVisible( obj ) ? iconWidth : iconWidth / 2
		);
	}
	...
	public void AddObject(LocalizedObject obj)
	{
		...
		LineData line = new LineData( GetMarkerColor(obj) );
			
		WireframeRender.Instance.linesUI.Add(line);
		knownObjectToMarker.Add(obj, line );
	...
	{
		if( knownObjectToMarker.ContainsKey(obj) ) {
			WireframeRender.Instance.linesUI.Remove(knownObjectToMarker[obj]);
			knownObjectToMarker.Remove(obj);
		}
	}
	...
	// {
	// 	HashSet<LocalizedObject> removedObjects = new HashSet<LocalizedObject>(knownObjectToMarker.Keys);
	...
	// 		//marquage des marqueurs à supprimer
	// 		removedObjects.Remove(currentObj);
	// 	}
		
	// 	foreach( LocalizedObject removedObject in removedObjects ) {
			
	// 		knownObjectToMarker.Remove(removedObject);
	// 	}
	// }
