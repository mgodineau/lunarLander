...
public class BetterMesh
{
	...
	public void RemoveDuplicates() {
		...
		Dictionary<Vertex, LinkedList<Vertex>> vertexToDuplicates = new Dictionary<Vertex, LinkedList<Vertex>>();
		for( int i=0; i<_vertices.Count; i++ ) {
			for( int j=i+1; j<_vertices.Count; j++ ) {
				if( _vertices[i].position != _vertices[j].position ) {
					continue;
				}
				if( !vertexToDuplicates.ContainsKey(_vertices[i]) ) {
					vertexToDuplicates.Add(_vertices[i], new LinkedList<Vertex>());
				}
				vertexToDuplicates[_vertices[i]].AddLast(_vertices[j]);
			}
		}
		
		
		foreach( Vertex vertex in vertexToDuplicates.Keys ) {
			foreach( Vertex duplicate in vertexToDuplicates[vertex] ) {
				ReplaceVertex(duplicate, vertex);
			}
		}
	...
	private void ReplaceVertex( Vertex oldVertex, Vertex newVertex ) {
		_vertices.Remove(oldVertex);
		foreach( Triangle tri in _triangles ) {
			tri.ReplaceVertex(oldVertex, newVertex);
		}
		foreach( Edge edge in _edges ) {
			edge.ReplaceVertex(oldVertex, newVertex);
		}
